...
import { findIssues, suggestOrder } from '../lib/schedule'
import { computeLegs, totalDistanceKm, TRANSPORT_LABEL } from '../lib/geo'
...
export default function PrefsScreen({ courseId }: { courseId?: string }) {
  ...
  const suggestion = useMemo(() => {
    ...
    const next = suggestOrder(course.places, prefs.transport)
  ...
  return (
    <div className="screen">
      ...
      <div className="scroll pad">
        ...
        {issues.length === 0 ? (
          <div className="banner" style={{ background: 'var(--ok-soft)', color: 'var(--ok)' }}>
        ...
        {!suggestion || suggestion.same ? (
          <div className="banner">
            지금 순서보다 거리가 짧은 대안을 찾지 못했어요. 조건을 완화하거나 장소를 조정해보세요.
          </div>
        ) : (
          <div className="card">
            <div className="bold small" style={{ marginBottom: 6 }}>
              순서와 이동수단을 조정하면 거리 {suggestion.before.toFixed(1)}km → {suggestion.after.toFixed(1)}km
            </div>
